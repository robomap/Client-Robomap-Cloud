...
import temp.model.SongsManager;
...
public class Reproductor implements BasicPlayerListener {
	...
	private boolean selectCanciones = false;
	boolean newSong = false;
	private String song;


	public Reproductor(MainView mainView, SongsManager songsManager, String username) {
	...
	public boolean loadSong(String song) { 
		...
		try {
			if(song.equals("selectCanciones")){
				selectCanciones = true;
				player.open(new File("./music/"+username+"ActualSong.mp3"));
				newSong = true;
			}else{
				this.song = song;
				selectCanciones = false;
				player.open(new File(song));
				newSong = true;
			}
		} catch (BasicPlayerException | NullPointerException e) {
			error = true;
			mainView.setProgressBar(0, 0, 0);
			estado = "error";
		}
		return error;
	}
	...
	public void opened(Object arg0, Map map) {
		...
		try {
			if(selectCanciones){
				baseFileFormat = AudioSystem.getAudioFileFormat(new File("./music/"+username+"ActualSong.mp3"));
			}else{
		...
		if(map.containsKey("audio.length.bytes")){
			...
			mainView.setMaximumProgressBar(((int)(long_bytes))*1000);
		}
	...
	public void progress(int bytesread,long microsecond,byte[] pcmdata, Map properties){
		...
		mainView.setProgressBar(progressNow, getTotalTime(), getTimeRead());
		if((getTimeRead() == getTotalTime()) && newSong){
			if(selectCanciones){
				stop();
